      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(
            ...
            style: TextStyle(color: Colors.black), // 타이틀 텍스트 색상을 흰색으로 변경
          ),
...
//삭제 함수
  ...
  Future<void> _deleteUserAccount() async {
    ...
    final token = prefs.getString('jwtToken');
    final username = prefs.getString('username');
    final fetchUrl = Uri.parse('http://10.0.2.2:8000/users/$username/');
...
//
  Future<void> _fetchUserInfo() async {
    try {
      ...
      final url = Uri.parse('http://10.0.2.2:8000/user/profile/'); // 백엔드 URL
