...
class Quiz {
  ...
  factory Quiz.fromJson(Map<String, dynamic> json) {
    return Quiz(
      ...
      questionText:
          json['questionText'] as String? ?? '', // null이면 빈 문자열을 기본값으로 사용
...
// Quiz 서비스
class QuizService {
  final String baseUrl = 'http://10.0.2.2:8080/api/quizzes';
  ...
  Future<List<Quiz>> fetchQuizzes() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Quiz> quizzes =
          body.map((dynamic item) => Quiz.fromJson(item)).toList();
      return quizzes;
    } else {
  ...
  Future<void> deleteAllQuizResponses() async {
    final response = await http
        .delete(Uri.parse('http://10.0.2.2:8080/api/quiz-responses/all'));
    if (response.statusCode != 200) {
...
class _QuizManagerScreenState extends State<QuizManagerScreen> {
  ...
  void _showAddEditQuizDialog({BuildContext? context, Quiz? quiz}) {
    ...
    final questionTextController =
        TextEditingController(text: quiz?.questionText ?? '');
    ...
    final difficultyController =
        TextEditingController(text: quiz?.difficulty ?? '');
    final scoreController =
        TextEditingController(text: quiz?.score.toString() ?? '');
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: TextFormField(
                          ...
                          decoration:
                              InputDecoration(hintText: "Question Text"),
                          maxLines: null,
                          validator: (value) {
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: TextFormField(
                          ...
                          decoration:
                              InputDecoration(hintText: "1. apple 2. 바나나"),
                          maxLines: null,
                        ),
                      ),
                    ],
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: TextFormField(
                          ...
                          decoration:
                              InputDecoration(hintText: "Correct Answer"),
                          maxLines: null,
                          validator: (value) {
                            if (value == null || value.isEmpty) {
                              return '정답을 입력해주세요';
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            quiz.options.isNotEmpty
                ? infoSection('보기', quiz.options, Colors.green[50]!)
                : Container(),
